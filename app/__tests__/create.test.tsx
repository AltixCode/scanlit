...
describe('the create screen', () => {
  it('renders a code once there is something to encode', async () => {
    ...
    await fireEvent.changeText(getByLabelText(t('createLabel')), 'https://example.com');

    await waitFor(() =>
      expect(getByLabelText(t('codeFor', { text: 'https://example.com' }))).toBeTruthy(),
    );
  });
  ...
  it('tells a free user that batches are one at a time, and only encodes one', async () => {
    const { getByLabelText, getByText, queryByLabelText } = await renderWithProviders(<Create />);
    expect(getByText(t('batchLockedNote'))).toBeTruthy();

    await fireEvent.changeText(getByLabelText(t('createLabel')), 'one\ntwo\nthree');
    await waitFor(() => expect(getByLabelText(t('codeFor', { text: 'one' }))).toBeTruthy());
  ...
  it('encodes a whole batch for a paying user', async () => {
    ...
    await waitFor(() => expect(getByLabelText(t('codeFor', { text: 'three' }))).toBeTruthy());
  });
  ...
  it('sends a free user tapping a colour to the paywall, changing nothing', async () => {
    ...
    await fireEvent.press(getByLabelText(`${locked.name} — ${t('lockedTitle')}`));
